...
export default function Reports() {
  ...
  const stats = useMemo(() => {
    ...
    const totalCollected = students.reduce(
      (sum, s) => sum + (s.paymentDetails?.paidAmount || 0),
      0,
    );
  ...
  const handleDownloadFullReport = () => {
    ...
    // Export as CSV
    const csvData = Object.entries(reportData["Summary Statistics"]).map(
      ([key, value]) => ({
        Metric: key,
        Value: value,
      }),
