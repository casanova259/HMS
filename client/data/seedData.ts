...
const FIRST_NAMES = [
  'Rajesh', 'Priya', 'Amit', 'Sneha', 'Vikram', 'Ananya', 'Arjun', 'Neha',
  'Rohan', 'Divya', 'Aditya', 'Pooja', 'Nikhil', 'Sara', 'Varun', 'Isha',
];

const LAST_NAMES = [
  'Kumar', 'Singh', 'Patel', 'Gupta', 'Sharma', 'Verma', 'Rao', 'Nair',
  'Chatterjee', 'Mishra', 'Joshi', 'Iyer', 'Menon', 'Desai', 'Bhat', 'Saxena',
];

const DEPARTMENTS: Array<'CSE' | 'ECE' | 'ME' | 'CE'> = ['CSE', 'ECE', 'ME', 'CE'];
const BLOCKS: Array<'A' | 'B' | 'C'> = ['A', 'B', 'C'];
const FLOORS: Array<'Ground' | '1st' | '2nd' | '3rd'> = ['Ground', '1st', '2nd', '3rd'];
const CAPACITIES: Array<'Single' | 'Double' | 'Triple'> = ['Single', 'Double', 'Triple'];
  ...
  floors.forEach((floor) => {
    blocks.forEach((block) => {
      for (let i = 0; i < 10; i++) {
        ...
        const capacityNum = capacity === 'Single' ? 1 : capacity === 'Double' ? 2 : 3;
        const status = Math.random() > 0.3 ? 'Occupied' : Math.random() > 0.5 ? 'Empty' : 'Maintenance';
        const occupancy = status === 'Empty' ? 0 : Math.floor(Math.random() * capacityNum) + 1;
        ...
        rooms.push({
          ...
          maintenanceIssue: status === 'Maintenance' ? 'Plumbing issue in bathroom' : undefined,
...
const generateStudents = (rooms: Room[]): Student[] => {
  ...
  for (let i = 0; i < 80; i++) {
    ...
    const occupiedRooms = rooms.filter((r) => r.status === 'Occupied' && r.occupancy < parseInt(r.capacity.charAt(0)));
    const room = occupiedRooms.length > 0 ? occupiedRooms[Math.floor(Math.random() * occupiedRooms.length)] : rooms[Math.floor(Math.random() * rooms.length)];
    ...
    students.push({
      ...
      dob: new Date(1998 + Math.floor(Math.random() * 6), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1).toISOString(),
      ...
      paymentDetails:
        paymentStatus === 'Paid'
          ? {
              ...
              paidAmount: 30000,
              paidDate: new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString(),
            }
          : { paidAmount: 0 },
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
...
const generateMaintenanceRequests = (rooms: Room[]): MaintenanceRequest[] => {
  ...
  const categories: Array<'Electrical' | 'Plumbing' | 'Furniture' | 'Cleaning' | 'Other'> = [
    'Electrical',
    'Plumbing',
    'Furniture',
    'Cleaning',
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const reportedDate = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000);

    requests.push({
      id: generateId(),
      title: ['Broken window', 'Leaking tap', 'Faulty light', 'Door hinge broken', 'Water damage'][Math.floor(Math.random() * 5)],
      ...
      assignedTechnician: status !== 'Pending' ? `Technician ${Math.floor(Math.random() * 10)}` : undefined,
      startedDate: status !== 'Pending' ? new Date(reportedDate.getTime() + 24 * 60 * 60 * 1000).toISOString() : undefined,
      estimatedCompletion: status !== 'Pending' ? new Date(reportedDate.getTime() + 5 * 24 * 60 * 60 * 1000).toISOString() : undefined,
      resolvedDate: status === 'Resolved' ? new Date(reportedDate.getTime() + 4 * 24 * 60 * 60 * 1000).toISOString() : undefined,
      resolutionNotes: status === 'Resolved' ? 'Issue fixed successfully' : undefined,
      progressPercentage: status === 'Pending' ? 0 : status === 'In Progress' ? Math.floor(Math.random() * 100) : 100,
      photosCount: Math.floor(Math.random() * 4),
      condition: ['Critical', 'Moderate', 'Minor'][Math.floor(Math.random() * 3)],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    });
  }
...
const generateComplaints = (students: Student[]): Complaint[] => {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    complaints.push({
      ...
      status,
      reportedDate: reportedDate.toISOString(),
      resolvedDate: status === 'Resolved' ? new Date(reportedDate.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString() : undefined,
...
const generateMenus = (): WeeklyMenu[] => {
  ...
  const createMealType = () => ({
    Breakfast: [{ name: 'Paratha, Butter, Tea', time: '8:00 AM', dietary: 'Veg' as const, allergens: ['dairy'] }],
    Lunch: [{ name: 'Rice, Daal, Paneer Curry, Roti', time: '12:30 PM', dietary: 'Veg' as const, allergens: ['dairy'] }],
    Snacks: [{ name: 'Tea, Samosa', time: '4:00 PM', dietary: 'Veg' as const, allergens: [] }],
    Dinner: [{ name: 'Roti, Chicken Curry, Rice', time: '7:30 PM', dietary: 'Non-veg' as const, allergens: [] }],
  });

  const currentWeek = Math.floor((Date.now() - new Date(2024, 0, 1).getTime()) / (7 * 24 * 60 * 60 * 1000));
...
const generateFoodRequests = (): FoodRequest[] => {
  ...
  const dishes = ['Samosa', 'Biryani', 'Momos', 'Pasta', 'Pizza', 'Ice Cream', 'Chocolate Cake', 'Sushi'];

  for (let i = 0; i < 10; i++) {
    const createdDate = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000);
    const closingDate = new Date(createdDate.getTime() + 7 * 24 * 60 * 60 * 1000);

    requests.push({
      id: generateId(),
      dishName: dishes[Math.floor(Math.random() * dishes.length)],
      description: 'A delicious and popular dish that students love',
      mealType: ['Breakfast', 'Lunch', 'Snacks', 'Dinner'][Math.floor(Math.random() * 4)] as any,
...
const generateAnnouncements = (): Announcement[] => {
  ...
  const types: Array<'General' | 'Urgent' | 'Event' | 'Maintenance' | 'Notice'> = ['General', 'Urgent', 'Event', 'Maintenance', 'Notice'];
...
const generateActivities = (): Activity[] => {
  ...
  const types = ['Student Allocated', 'Payment Received', 'Maintenance Reported', 'Complaint Filed', 'Announcement Posted'];
